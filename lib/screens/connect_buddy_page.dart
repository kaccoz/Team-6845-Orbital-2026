...
class _ConnectBuddyPageState extends State<ConnectBuddyPage> {
  ...
  void _submitBuddyCode() async {
    ...
    if (inputCode.isEmpty || inputCode.length != 6) {
      _showSnackBar('Please enter a valid 6-character UID.', isError: true); 
      return;
    }
  ...
  void _unlinkBuddy() async {
    ...
    try {
      ...
      setState(() => _currentBuddyUid = null);
    } catch (e) {
    ...
    if (_currentBuddyUid != null) {
      bodyContent = FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          final buddyData = snapshot.data![1].data() as Map<String, dynamic>?;
          
          final myStreakData = snapshot.data![2].data() as Map<String, dynamic>?;
          final buddyStreakData = snapshot.data![3].data() as Map<String, dynamic>?;

          return BuddiesPage(
            currentUserName: myData?['username'] ?? 'Me',
            currentUserPfpBase64: myData?['photoUrl'],
            currentUserStreak: (myStreakData?['dates'] as List<dynamic>?)?.length ?? 0, 
            buddyName: buddyData?['username'] ?? 'Buddy',
            buddyId: _currentBuddyUid!,
            buddyPfpBase64: buddyData?['photoUrl'],
            buddyStreak: (buddyStreakData?['dates'] as List<dynamic>?)?.length ?? 0,
            onUnlink: _unlinkBuddy,
          );
        },
      );
    } else {
      // (The rest of your UI for the input field remains the same)
      bodyContent = _buildInputView();
    }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const Text('Connect with a Friend', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: primaryBrown)),
              ...
              child: Column(
                children: [
                  TextField(
                    ...
                    maxLength: 6,
                    decoration: const InputDecoration(hintText: 'X7R2A9', filled: true, fillColor: backgroundColor),
                  ),
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: _isSubmittingCode ? null : _submitBuddyCode,
                    child: const Text('Link Buddy'),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildBottomNavigationBar(BuildContext context) {
    return Theme(
      ...
      child: BottomNavigationBar(
        ...
        currentIndex: 2,
        onTap: (index) {
          if (index == 0) Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => HomePage()), (route) => false);
          if (index == 1) Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => HabitsPage()));
          if (index == 3) Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const ProfilePage()));
        },
        items: const [
